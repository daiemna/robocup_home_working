...
ros::Publisher recognized_face_name_pub_;
            ...
            const face_recognition::FaceRecognitionResultConstPtr& result)
{
  ...
  if( result->order_id==0) {
    ...
    std_msgs::String recognized_name_msg;
    recognized_name_msg.data = result->names[0].c_str();
    recognized_face_name_pub_.publish(recognized_name_msg);
  }
...
void feedbackCb(const face_recognition::FaceRecognitionFeedbackConstPtr& feedback)
{
  ...
  if(feedback->order_id==1 ) {
    ...
    std_msgs::String recognized_name_msg;
    recognized_name_msg.data = feedback->names[0].c_str();
    recognized_face_name_pub_.publish(recognized_name_msg);
  }       
...
int main (int argc, char **argv)
{
  ...
  recognized_face_name_pub_ = n.advertise<std_msgs::String>("recognized_face_name", 1000);
  sleep(1.0);
